...
base_price = 25; % 基础电价（元/kWh）

%% 2. 初始化 AC 参数
acFile = 'actest2.xlsx'; % 确保 AC_template1.xlsx 在路径中
fprintf('正在初始化空调参数...\n');
try
...
% [新增] 创建 AC 数据的纯净备份，用于在循环中重置
ACs_Backup = ACs;

%% 3. 激励响应参数 & 批量仿真设置
p_min = 12; p_max = 40; p_min_prime = 8; p_max_prime = 32; T_set_max = 3;

% [新增] 定义要仿真的价格列表 (0-50, 均匀10个点)
price_list = linspace(0, 40, 10);
...
fprintf('即将开始批量仿真，共 %d 个价格点: %s\n', num_prices, num2str(price_list));

% ====================== 批量仿真主循环开始 ======================
for p_idx = 1:num_prices

    current_p = price_list(p_idx);
    fprintf('\n==========================================================\n');
    fprintf('== [进度 %d/%d] 正在仿真价格场景: %.2f 元 ==\n', p_idx, num_prices, current_p);
    fprintf('==========================================================\n');
    ...
    Kp_inverter = 0.9 / mean_beta;
    Ki_inverter = 0.9 / mean_beta;
    ...
    results.current_price = current_p; % 保存当前价格
    ...
    % 保存文件 (动态文件名)
    mat_filename = sprintf('AC_Stateful_Simulation_Results_Price_%.1f_pi_8am_04.mat', current_p);
    output_mat_path = fullfile(output_dir, mat_filename); % 组合文件夹和文件名
    save(output_mat_path, 'results', '-v7.3');
    fprintf('  完整数据已保存至: %s\n', output_mat_path);
    % --- [Step 5.8 修改结束] ---
