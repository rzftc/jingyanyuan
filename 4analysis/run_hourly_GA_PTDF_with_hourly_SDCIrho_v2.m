% run_hourly_GA_PTDF_with_hourly_SDCIrho.m (版本 11: 引入小时级MILP求解器)
function run_hourly_GA_PTDF_with_hourly_SDCIrho_v2()
    ...
    chunk_load_config = [
        ...
        struct('file', 'chunk_results/results_chunk_2.mat', 'ac_rows', 100, 'ev_rows', 1000)
        % 您可以在此处添加更多文件:
    ...
    for h = 1:num_hours
        ...
        % --- 上调调度 (调用新函数) ---
        [u_ac_up_h, u_ev_up_h, cost_up_h_total, flag_up] = solve_hourly_dispatch_ptdf_hourly( ...
        ...
        % --- 下调调度 (调用新函数) ---
        [u_ac_down_h, u_ev_down_h, cost_down_h_total, flag_down] = solve_hourly_dispatch_ptdf_hourly( ...
