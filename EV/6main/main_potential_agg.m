%% main_potential.m (已修改为使用聚合模型和 _new 函数计算潜力)
clc; clear; close all;
rng(2024);
...
simulation_end_hour   = 30; % 仿真到第二天早上6点结束
dt = dt_short / 60;       % 短时间步长 (小时)
dt_minutes = dt_short;    % 短时间步长 (分钟)
dt_long_minutes = dt_long;% 长时间步长 (分钟)
t_adj = dt_long / 60;     % 调节时长 (小时) - 用于 calculateEVAdjustmentPotentia_new
...
results = struct(...
    ...
    'P_base',        zeros(1, total_steps), ... % 这个P_base是聚合后的吗?
...
% --- E_tar, tau_rem, E_exp, E_actual, E_current, P_current 初始化 ---
E_tar_vec = max(E_tar_set_vec, E_ini_vec); % 仅用于计算初始 tau_rem
...
for long_idx = 1:num_long_steps
    ...
    for short_idx = 1:num_short_per_long
        ...
        % % --- 移除用于个体潜力的临时变量 ---
        % temp_delta_p_plus = zeros(num_evs, 1);
        % temp_delta_p_minus = zeros(num_evs, 1);
        % % --- 移除结束 ---
        ...
        % clear temp_delta_p_plus temp_delta_p_minus;
        % % --- 移除结束 ---
        ...
        if ~isempty(active_participating_indices)
            ...
            E_current_agg = sum([active_EVs.E_actual]); % 使用更新后的 E_actual 聚合
            % 平均离网时间 (小时) - 简化处理
            t_dep_agg_h = mean([active_EVs.t_dep]) / 60;
            % 总额定功率
            p_on_agg = sum([active_EVs.P_N]);
            % 总基线功率
            P_base_agg = sum(arrayfun(@(ev) ev.P_base_sequence(step_idx), active_EVs));
            % 平均效率 - 简化处理
            eta_agg = mean([active_EVs.eta]);

            % 3. 调用 calculateEVAdjustmentPotentia_new 函数
...
%% ===================== 结果保存与可视化 =====================
outputFileName = 'main_potential_aggregated_newlogic_results.mat'; % 定义新的输出文件名
fprintf('\n正在保存包含聚合EV潜力(基于_new逻辑)的结果到 %s ...\n', outputFileName);
try
...
% --- 可视化聚合潜力 ---
figure;
plot(time_points_absolute, results.EV_Up, 'r-', 'DisplayName', 'Aggregated Up Potential (Model + New Logic)');
hold on;
plot(time_points_absolute, results.EV_Down, 'b-', 'DisplayName', 'Aggregated Down Potential (Model + New Logic)');
xlabel('Time (hours)');
ylabel('Potential (kW)');
...
yyaxis left;
plot(time_points_absolute, S_original_vis, 'b-', 'LineWidth', 1.5, 'DisplayName', '原始SOC (S_{original})');
hold on;
plot(time_points_absolute, S_mod_vis, 'r--', 'LineWidth', 1.5, 'DisplayName', '修正SOC (S_{modified})');
ylabel('SOC'); ylim([-1.1 1.1]); grid on;
yyaxis right;
plot(time_points_absolute, P_current_vis, 'g-.', 'LineWidth', 1.5, 'DisplayName', ['EV ', num2str(selected_ev), ' 功率 (P_{current})']);
ylabel('功率 (kW)');
