function generateEVParameters_real(filePath, numEV, areaRatio)
    % 生成严格匹配车型参数的EV数据（支持区域比例调节）
    % 输入参数：
    %   filePath   - 输出文件路径
    %   numEV      - EV总数（默认1000）
    %   areaRatio  - 居民区比例（默认0.5）
    
    % 参数校验
    if nargin < 3
        areaRatio = 0.5;  % 默认各占50%
    end
    if nargin < 2
        numEV = 1000;
    end
    assert(areaRatio >=0 && areaRatio <=1, '区域比例必须在0-1之间');
    ...
    %% 区域分配（按指定比例）
    isResidential = rand(numEV,1) < areaRatio;
