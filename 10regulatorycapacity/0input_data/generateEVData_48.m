%% generateEVData.m
function generateEVData_48(num_devices_ev, area_type, selected_models)
    % 参数验证
    if nargin < 3
    ...
    for i = 1:num_devices_ev
        ...
        while ~valid
            ...
            if (duration < 1e-6 && abs(E_tar(i) - E_in(i)) < 1e-3) || ...
               (duration >= 1e-6 && E_tar(i) >= E_in(i) - 1e-3 && required_power_check <= p_on(i) + 1e-3 && p_on(i) > 1e-3) || ...
               (duration >= 1e-6 && abs(E_tar(i) - E_in(i)) < 1e-3 && p_on(i) < 1e-3) 
                valid = true;
            end
        end
    ...
    filename_to_save = sprintf('EV_%s.xlsx', area_type); 
    writetable(struct2table(evData), fullfile(saveDir, filename_to_save));
    disp(['EV数据文件已生成：' filename_to_save]);
end

%% 应用实例
% generateEVData_48(100, 'residential')
% generateEVData_48(50, 'workplace', {'特斯拉Model Y'})