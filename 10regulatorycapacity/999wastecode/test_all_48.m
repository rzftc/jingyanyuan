%% 全功能虚拟电厂调节潜力分析系统（48小时版本）
clear; close all; clc;

%% 1. 系统初始化
rng(2023);                                      % 固定随机种子
T_total = 48;                                   % 总时长改为48小时
dt = 0.05;                                       
time_points = 0:dt:T_total;
base_price = 30;                                

%% 2. 初始化参数（生成函数需要同步修改）
generateExampleExcel_real_48(10,10, 0.6);        % 生成48小时兼容数据
...
for i = 1:num_AC
    ...
    [~, ~, deltaT] = incentiveTempAC(...        % 获取温度偏移量
        ACs(i).p_incentive, p_min, p_max, p_min_prime, p_max_prime, T_set_max);
    
    ACs(i).ptcp = (rand() < participation);     % 生成参与状态
    ...
    % 基础温度波动（含日夜变化）
    base_temp = ACs(i).Tset + 4*sin(2*pi*time_points/48); 
