% optimize_hourly_participation_GA_constrained_ptdf_FIXED.m 
% (FIXED: 传递成本参数)
function [n_ac_opt, n_ev_opt] = optimize_hourly_participation_GA_constrained_ptdf_FIXED( ...
    ...
    % [!!! 关键修改 2b !!!] 传递网络参数和成本参数给适应度函数
    fitness_fcn = @(x) hourly_ga_fitness_constrained_FIXED(x, ...
    ...
    if nargin > 16 && ~isempty(ga_options) && iscell(ga_options) % 修正 nargin 检查
