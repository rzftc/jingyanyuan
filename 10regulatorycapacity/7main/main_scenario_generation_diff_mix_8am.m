...
parfor s = 1:num_scenarios
    ...
    % --- AC 仿真 (接收基线) ---
    [ac_up, ac_down, ac_base] = run_AC_simulation_MC_8am(current_seed, temp_ac_file);
    
    % --- EV 仿真 (接收基线) ---
    % 注意：ev_power_profile 即为该场景下的 EV 基线充电负荷
    [ev_up, ev_down, ev_power_profile] = run_EV_simulation_MC_8am(current_seed, temp_ev_file);
...
save('reliable_regulation_domain_1000_mix_pbase_8am.mat', ...
