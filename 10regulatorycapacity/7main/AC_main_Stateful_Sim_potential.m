...
% 功能：AC集群状态化仿真与调节潜力对比分析（单体累加 vs 聚合模型）
...
T_ja_min = max_Tset_all + 0.1;  % 保证 T_ja > T_set
...
for t_idx = 1:T_steps_total
    ...
    % 功率约束潜力 (基于物理基线)
    P_agg_power_up = P_base_instant_agg;  % P_max - P_base = 2*P_base - P_base
    P_agg_power_down = -P_base_instant_agg; % P_min - P_base = 0 - P_base
...
xlabel('时间 (h)'); ylabel('功率 (kW)'); title('图1: 聚合响应功率 vs 电网指令');
legend; grid on; xlim([0, 24]);
...
xlabel('时间 (h)'); ylabel('SOC'); title('图2: 单体SOC vs 聚合SOC');
xlim([0, 24]); ylim([-0.1, 1.1]); grid on;

% 图6: 聚合功率对比
figure('Name', '聚合功率对比', 'Position', [100 600 1000 450]);
plot(time_points, Agg_Baseline_Power, 'b--', 'LineWidth', 2, 'DisplayName', '聚合基线功率'); hold on;
plot(time_points, Agg_Total_Power, 'r-', 'LineWidth', 2, 'DisplayName', '聚合总制冷功率');
xlabel('时间 (h)'); ylabel('功率 (kW)'); title('图6: 基线功率 vs 总制冷功率');
legend; grid on; xlim([0, 24]);

% 图7: 调节潜力对比 (核心图表)
figure('Name', '调节潜力对比', 'Position', [100 700 1000 450]);
plot(time_points, Agg_P_Potential_Up_History, 'b-', 'LineWidth', 1.5, 'DisplayName', '单体累加上调 (ΣP_{+,j})'); hold on;
plot(time_points, Agg_P_Potential_Down_History, 'r-', 'LineWidth', 1.5, 'DisplayName', '单体累加下调 (ΣP_{-,j})');
plot(time_points, Agg_Model_Potential_Up_History, 'b--', 'LineWidth', 2, 'DisplayName', '聚合模型上调 (P_{+,agg})');
plot(time_points, Agg_Model_Potential_Down_History, 'r--', 'LineWidth', 2, 'DisplayName', '聚合模型下调 (P_{-,agg})');
xlabel('时间 (h)'); ylabel('潜力 (kW)'); title('图7: 调节潜力对比 (单体累加 vs 聚合模型)');
legend; grid on; xlim([0, 24]);
