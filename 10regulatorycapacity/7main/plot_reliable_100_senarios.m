...
%       [新增] 强制将EV在特定时段（上调8-18h，下调8-12h）的调节能力置零。
...
% 提取选定场景数据
% 注意：假设原始数据单位需要乘5 (参考原plot文件逻辑: 5*Scenarios...)
S_AC_Up   = Scenarios_AC_Up(:, selected_indices);
S_AC_Down = Scenarios_AC_Down(:, selected_indices);
S_EV_Up   = Scenarios_EV_Up(:, selected_indices)*2.5;
S_EV_Down = Scenarios_EV_Down(:, selected_indices) * 2.5;

fprintf('已选择 %d 个场景进行概率边界分析。\n', target_scenarios);

%% 3. 时间轴平移与数据重组 (08:00-32:00 -> 00:00-24:00)
% 原始逻辑：仿真从 08:00 (t=8) 开始，到次日 08:00 (t=32)
% 目标逻辑：将 [24, 32] 区间的数据移动到 [0, 8]，拼接在 [8, 24] 之前

fprintf('正在执行时间轴平移 (25-32h -> 0-8h)...\n');

% 找到分割点：24:00
% 注意：time_points >= 24 的部分对应次日 00:00 - 08:00
idx_part1 = find(time_points >= 24); % 次日凌晨部分
idx_part2 = find(time_points < 24);  % 当日白天部分

% 构造新的 0-24h 时间轴
% part1 减去 24 小时，变成 0-8
% part2 保持不变，是 8-24
...
%% === [新增逻辑] 强制置零特定时间段的EV数据 ===
fprintf('正在处理EV时段约束：上调(8:00-18:00置零)，下调(8:00-12:00置零)...\n');
...
% 我们使用排序法计算百分位数，避免依赖 Statistics Toolbox
% 排序维度：dim=2 (沿场景方向)

fprintf('正在计算 100%%, 95%%, 80%% 置信边界...\n');

% 辅助函数：计算边界
% 对于上调 (正值)：置信度越高，取值越小（保守）。
%   100% -> min, 95% -> 5th percentile, 80% -> 20th percentile
% 对于下调 (负值)：置信度越高，取值越大（越接近0，保守）。
%   100% -> max, 95% -> 95th percentile, 80% -> 80th percentile
...
% 1. 绘制背景场景 (极淡，展示离散性)
h_bg_up = plot(new_time_points, AC_Up_Shifted, 'Color', [0.6, 0.8, 1, 0.08], 'HandleVisibility', 'off');
h_bg_down = plot(new_time_points, AC_Down_Shifted, 'Color', [1, 0.6, 0.6, 0.08], 'HandleVisibility', 'off');
