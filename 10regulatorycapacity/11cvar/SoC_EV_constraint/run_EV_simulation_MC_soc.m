function [EV_Up_Sum, EV_Down_Sum, EV_Power_Sum, AggParams_EV] = run_EV_simulation_MC_soc(random_seed, evFileName)
    ...
    Agg_Cap_trace = zeros(total_steps, 1);
    ...
    for long_idx = 1:num_long_steps
        ...
        for short_idx = 1:num_short_per_long
            ...
            % 计算当前时刻的在线总容量
            if any(is_connected)
                Current_Online_Cap = sum(C_vec(is_connected)); % 仅累加当前在线且参与聚合的车辆容量
            else
                Current_Online_Cap = 0;
            end
            Agg_Cap_trace(step_idx) = Current_Online_Cap; % 记录到轨迹中
            % ===================================
    ...
    % 输出时变容量轨迹，替代原来的静态总和
    AggParams_EV.C_total_trace = Agg_Cap_trace; 
    % (可选) 保留原有的静态总容量作为参考，防止其他代码报错，但改个名字或保持原样
    AggParams_EV.C_total_static = sum(C_vec(ptcp_vec));
end