function [AC_Up_Sum, AC_Down_Sum, AC_Base_Sum, AggParams] = run_AC_simulation_MC_soc(random_seed, acFileName)
    % run_AC_simulation_MC
    % 修改：
    % 1. 增加 AC_Base_Sum 输出，用于记录空调集群的基线负荷
    % 2. 增加 AggParams 输出，用于随机优化模型中的灰盒约束
    
    %% 1. 系统初始化
    if nargin > 0
        rng(random_seed, 'Threefry');
    end
    ...
    %% 2. 初始化 AC 参数
    try
        ACs = initializeACsFromExcel(acFileName);
    catch ME
    ...
    if num_AC_participating == 0
        ...
        AC_Base_Sum = zeros(T_steps_total, 1); % 新增
        % 防止 AggParams 未定义报错
        AggParams.A = 0; AggParams.B = 0; AggParams.C = 0; 
        return;
    end
    ...
    Agg_P_Base_History = zeros(T_steps_total, 1); % 新增

    %% 5. 主时间循环
    for t_idx = 1:T_steps_total
        SOC_agg_t = mean(CURRENT_SOC_AC, 'omitnan');
        Delta_P_S_command = 0; % 基线模式
        ...
        parfor i = 1:num_AC_participating
            ...
            temp_P_base_agg = temp_P_base_agg + P_base_i; % 累加基线
        ...
        Agg_P_Base_History(t_idx) = temp_P_base_agg; % 记录基线
        CURRENT_SOC_AC = temp_SOC_next;
    end
    
    AC_Up_Sum = Agg_P_Potential_Up_History;
    AC_Down_Sum = Agg_P_Potential_Down_History;
    AC_Base_Sum = Agg_P_Base_History; % 返回基线
    % AggParams 已经在上面计算并在此函数结束时自动返回
end