...
%   通过 risk_p.tight_factor / risk_p.margin 控制“多保守”。
    ...
    % 修改逻辑：从“全天总能量约束”改为“逐时刻功率约束”
    % 约束: rho * (P_AC(t) + P_EV(t)) - eta - z_s <= rho * Capacity_s(t)
    % 这意味着 z_s 必须大于该场景下所有时刻中最严重的那个违约量 (Max over t)
    
    % S_AC 是 T x N_scenarios
    S_total = S_AC + S_EV;
    
    % tight_factor 和 margin 处理
    tight_factor = 1;
    ...
    S_limit = max(0, tight_factor * S_total - margin); % T x N矩阵
    
    % 右端项向量 b_cvar1: 长度 T * N_scenarios
    % 展开顺序：Matlab默认列优先，即 S_limit(:) 是 [t=1..T (s=1); t=1..T (s=2); ...]
    b_cvar1 = rho * S_limit(:); 
    
    num_con_cvar = T * N_scenarios;
    
    % 构造左端项 A_cvar1
    % 对应约束 k (1 到 num_con_cvar)
    % k 对应场景 s = ceil(k/T), 时间 t = mod(k-1, T) + 1
    
    % 辅助索引向量
    row_idx = (1:num_con_cvar)';
    
    % 时间 t 的索引：[1; 2; ... T; 1; 2; ... T; ...]
    t_idx = repmat((1:T)', N_scenarios, 1); 
    
    % 场景 s 的索引：[1...1 (T个); 2...2 (T个); ...]
    s_idx = ceil(row_idx / T); 
    
    % --- 1. P_AC 和 P_EV 的系数 (rho) ---
    % P_AC
    i_pac = row_idx;
    j_pac = idx_P_AC(t_idx)'; % 获取每个约束对应的 t 时刻的 P_AC 列号
    v_pac = repmat(rho, num_con_cvar, 1);
    
    % P_EV
    i_pev = row_idx;
    j_pev = idx_P_EV(t_idx)'; % 获取每个约束对应的 t 时刻的 P_EV 列号
    v_pev = repmat(rho, num_con_cvar, 1);
    
    % --- 2. eta 的系数 (-1) ---
    i_eta = row_idx;
    j_eta = repmat(idx_eta, num_con_cvar, 1);
    v_eta = repmat(-1, num_con_cvar, 1);
    
    % --- 3. z_s 的系数 (-1) ---
    i_z = row_idx;
    j_z = idx_z(s_idx)'; % 每个约束对应其所属场景 s 的变量 z_s
    v_z = repmat(-1, num_con_cvar, 1);
    
    % 拼装稀疏矩阵
    A_cvar1 = sparse([i_pac; i_pev; i_eta; i_z], ...
                     [j_pac; j_pev; j_eta; j_z], ...
                     [v_pac; v_pev; v_eta; v_z], ...
                     num_con_cvar, num_vars);
